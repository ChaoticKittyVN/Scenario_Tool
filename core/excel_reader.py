...
class ExcelFileManager:
    ...
    def load_excel(self, file_path: Path) -> Dict[str, pd.DataFrame]:
        ...
        if file_path in self._file_cache and self.cache_enabled:
            logger.info(f"从缓存加载Excel文件: {file_path}")
    ...
    def get_sheet(self, file_path: Path, sheet_name: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"获取工作表: {file_path} -> {sheet_name}, 形状: {df.shape}")
            return df
        except Exception as e:
            logger.error(f"获取工作表失败: {file_path} -> {sheet_name}, 错误: {e}")
            return pd.DataFrame()
...
class DataFrameProcessor:
    ...
    # TODO 暂无应用场景，待优化
    def filter_by_condition(self, df: pd.DataFrame, condition_func: Callable) -> pd.DataFrame:
        """
        基于条件函数过滤数据

        Args:
            df: 输入的DataFrame
            condition_func: 条件函数，接受一行数据返回布尔值
        ...
        try:
            ...
            filtered_df = df[mask].reset_index(drop=True)
            logger.info(f"条件过滤: 原始行数 {len(df)}, 过滤后 {len(filtered_df)}")
            return filtered_df
        except Exception as e:
