...
class TextGenerator(BaseSentenceGenerator):
    ...
    def process(self, data):
        ...
        printer_state = self.get_value("PrinterState", data)
        ...
        if printer_state != WindowMode.HIDE.value:
            if printer_state in [WindowMode.SHOW.value, WindowMode.SHOW_AND_HIDE.value] or printer or (transition in ["局部转场", "新场景", "立绘转场"] and transition_sub not in ["过渡", "开始"]):
                printer = self.translator.translate('Printer', printer)
                if self.exists_param("Printer", data):
                    printer_show += f" {printer}"
